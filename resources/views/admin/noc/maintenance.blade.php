<x-layouts.admin title="NOC - Maintenance">
    <div class="mb-6">
        <h2 class="text-2xl font-semibold">NOC - Maintenance</h2>
        <p class="text-gray-600 dark:text-gray-400">Kelola semua permintaan maintenance jaringan.</p>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-lg shadow mb-4">
        <div class="px-6 py-4 flex justify-between items-center">
            <div>
                <h3 class="text-lg font-semibold">Daftar Permintaan Maintenance</h3>
            </div>
            <div class="flex space-x-2">
                ...
                <div class="flex space-x-2">
                    <a href="{{ route('admin.noc.maintenance.export.excel') }}" class="btn-secondary flex items-center">
                    ...
                    <a href="{{ route('admin.noc.maintenance.export.csv') }}" class="btn-secondary flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    ...
                    <a href="{{ route('admin.noc.maintenance.create') }}" class="btn-primary flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <table
                            ...
                            <thead class="bg-gray-100 dark:bg-gray-700">
                                <tr
                                    ...
                                    <th class="px-3 py-3">Nomor Tracking</th>
                                    <th class="px-3 py-3">Nama Tenant</th>
                                    <th class="px-3 py-3">Email</th>
                                    <th class="px-3 py-3">Jenis Maintenance</th>
                                    <th class="px-3 py-3">Urgensi</th>
                                    <th class="px-3 py-3">Tanggal Mulai</th>
                                    ...
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-750 transition">
                                        ...
                                        <td class="px-3 py-2 text-center">
                                            <div class="flex justify-center items-center">
                                                {{ $request->nomor_tracking }}
                                                @if ($request->tingkat_urgensi == 'Critical')
                                                    <span
                                                        class="ml-2 px-2 py-1 text-xs font-bold bg-red-100 text-red-800 rounded-full">!</span>
                                                @endif
                                            </div>
                                        </td>
                                        <td class="px-3 py-2 text-left">{{ $request->nama_tenant }}</td>
                                        <td class="px-3 py-2 text-left">{{ $request->email ?? '-' }}</td>
                                        <td class="px-3 py-2 text-left">{{ $request->jenis_maintenance }}</td>
                                        <td class="px-3 py-2 text-center">
                                            <span
                            ...
                            {{ $request->tingkat_urgensi == 'Low' ? 'bg-green-100 text-green-800' : '' }}">
                                                {{ $request->tingkat_urgensi }}
                                            </span>
                                        </td>
                                        <td class="px-3 py-2 text-center whitespace-nowrap">
                                            {{ $request->tanggal_mulai ? $request->tanggal_mulai->format('d/m/Y') : '-' }}
                                        </td>
                                        <td class="px-3 py-2 text-center whitespace-nowrap">
                            ...
                            {{ $request->status == 'Ditolak' ? 'bg-red-100 text-red-800' : '' }}">
                                        ...
                                        <td class="px-3 py-2 text-center">
                                            <x-noc-crud-buttons :viewRoute="route('admin.noc.maintenance.show', $request->id)" :editRoute="route('admin.noc.maintenance.edit', $request->id)" :deleteRoute="route('admin.noc.maintenance.destroy', $request->id)" />
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                    ...
                    <div class="mt-6 flex flex-col md:flex-row justify-between items-center space-y-2 md:space-y-0">
                        <div class="text-sm text-gray-500 dark:text-gray-400">
                            Showing <span class="font-medium">1</span> to <span
                                class="font-medium">{{ $maintenance->count() }}</span> of <span
                                class="font-medium">{{ $maintenance->total() }}</span> results
                        </div>
